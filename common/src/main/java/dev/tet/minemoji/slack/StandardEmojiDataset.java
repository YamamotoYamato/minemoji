...
public final class StandardEmojiDataset {
	...
	private static final Logger LOGGER = Logger.getLogger(StandardEmojiDataset.class.getName());
	...
	private static Map<String, String> load() {
		...
		if (stream == null) {
			LOGGER.warning("Standard emoji dataset resource is missing: " + RESOURCE_PATH);
			return Map.of();
		}

		try (Reader reader = new InputStreamReader(stream, StandardCharsets.UTF_8)) {
			Map<String, String> parsed = GSON.fromJson(reader, MAP_TYPE);
			return normalizeNames(parsed == null ? Map.of() : parsed);
		} catch (IOException | RuntimeException exception) {
			LOGGER.log(Level.WARNING, "Failed to load standard emoji dataset from " + RESOURCE_PATH, exception);
			return Map.of();
		}
	}
	...
	private static Map<String, String> normalizeNames(Map<String, String> source) {
		...
		if (manBowing != null) {
			normalized.put("man-bowing", manBowing);
		}
		if (womanBowing != null) {
			normalized.put("woman-bowing", womanBowing);
		}
