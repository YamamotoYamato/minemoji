...
public final class SlackEmojiService {
	...
	private static final Logger LOGGER = Logger.getLogger(SlackEmojiService.class.getName());

	private final MinemojiConfig config;
	private final Runnable textureCacheClearer;
	...
	public SlackEmojiService(MinemojiConfig config, Runnable textureCacheClearer) {
		this.config = config;
		this.textureCacheClearer = Objects.requireNonNull(textureCacheClearer, "textureCacheClearer");
	}
	...
	private SlackEmojiRefreshResult refreshBlocking() {
		try {
			...
			this.lastError = message;
			LOGGER.log(Level.WARNING, "Failed to refresh Slack emoji", exception);
			return SlackEmojiRefreshResult.failure(message, this.emojis.size());
		} catch (RuntimeException exception) {
			String message = "Slack refresh failed: " + describeException(exception);
			this.lastError = message;
			LOGGER.log(Level.WARNING, "Failed to parse Slack emoji response", exception);
			return SlackEmojiRefreshResult.failure(message, this.emojis.size());
		}
	}
