...
public final class MinemojiConfig {
	...
	private static final Logger LOGGER = Logger.getLogger(MinemojiConfig.class.getName());
	...
	public static MinemojiConfig load(Path path) {
		...
		try (Reader reader = Files.newBufferedReader(path, StandardCharsets.UTF_8)) {
			...
			LOGGER.log(Level.WARNING, "Failed to load config from " + path, exception);
			MinemojiConfig config = new MinemojiConfig();
			config.path = path;
			return config;
		}
	...
	public void save() {
		...
		try {
			...
			LOGGER.log(Level.WARNING, "Failed to save config to " + this.path, exception);
		}
	}
