...
public final class MinemojiClient implements ClientModInitializer {
	...
	private final MinemojiConfig config = MinemojiConfig.load(FabricLoader.getInstance().getConfigDir().resolve(MOD_ID + ".json"));
	private final SlackEmojiService slackEmojiService = new SlackEmojiService(this.config);
	private final SlackEmojiTextureCache slackEmojiTextureCache = new SlackEmojiTextureCache(this.slackEmojiService);
			...
			ClientCommands.literal(MOD_ID)
				...
				.then(ClientCommands.literal("config")
					.executes(context -> {
						...
						minecraft.execute(() -> minecraft.setScreenAndShow(new MinemojiConfigScreen(null)));
						return Command.SINGLE_SUCCESS;
					})
				)
